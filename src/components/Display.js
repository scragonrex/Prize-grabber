...
const Display = (props) => {
    ...
    const fetchPrize = async () => {
        ...
        let prizeData=parseData.prizes;
    ...
    useEffect(() => {
        fetchPrize();
    }, [page,props.category,props.startyear,props.endyear]);
    const nextContent = () => {
        setPage(page + 1);
    ...
    return (
            <div className="container asset1">
                <div className="row">
                    <div className="col-2 border">Name</div>
                    <div className="col-2 border">Category</div>
                    <div className="col-1 border">Year</div>
                    <div className="col-7 border">Motivation</div>
                </div>
            ...
            <div className="button1 container">
                <button className="btn1" onClick={previousContent} disabled={true ? page <= 1 : false}>&laquo; Previous</button>
                <button className="btn1">{page}</button>
                <button disabled={true ? page * pageSize < prize.length : false} className="btn1" onClick={nextContent}>Next &raquo;</button>
            </div>
            </div>
